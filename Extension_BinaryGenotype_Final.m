...
mutationRate= 0.03; 
TournSize = 5;
BPar = 0.75;%0.6;%0.5;% 0.9%        %Introduce parasite mutation bias. 
...
StoreHostFitnessSim = zeros(NoHost,NoGenerations);
StoreParFitnessSim = zeros(NoPar, NoGenerations);
% SumH = zeros(NoHost,NoGenerations);
% SumP = zeros(NoPar,NoGenerations);
% MeanOnesH = zeros(1,NoGenerations);
% MeanOnesP = zeros(1,NoGenerations);


% TotalOnesHostIndivs = zeros(NoRuns,NoGenerations);
% TotalOnesParIndivs = zeros(NoRuns,NoGenerations);
...
%Need 2 populations of genotype zeros to start
    ...
    HostFitness = zeros(1,NoHost);
    HostFit = zeros(1,NoHost);
    
    [HostFit,HostFitness] = SimpleHostFitness(HostPop,NoHost,HostFitness,NoGenes,TargetString); %Call on host fitness function

    XScore = zeros(1,NoPar);
    ParasiteFit = zeros(1,NoPar);
    [ParFit] = SimpleParFitness(ParPop,NoPar,NoGenes,TargetString); %Find simple parasite fitness by counting ones
    ...
    for c = 1:NoGenerations
        %Cartlidge used a random seed (Same time each run) to start each run
        RelHostFitness = zeros(1,NoHost);   %Initialize relative host fitness Moved from outside main loop bc increaseing too uch
        ...
        for i = 1:NoHost
            ...
            [maxFitCalc,maxIndex] = max(ParasiteFit(TournComp));
            if maxFitCalc > HostFit(i) %select parasite as winner
                Win = TournComp(maxIndex); 
                HostWin = 0;
            elseif maxFitCalc == HostFit(i)
                if rand > 0.5
                    Win = TournComp(maxIndex); %If equal fitnesses, randomly select par or host as winner
                    HostWin = 0;
                else
                    Win = i; 
                    HostWin = 1;
            ...
            for e = 1:TournSize
                if HostFit(i) > ParasiteFit(TournComp(e)) % Add to relative fitness score
                    RelHostFitness(i) = RelHostFitness(i)+1;
                elseif HostFit(i) == ParasiteFit(TournComp(e))
                    RelHostFitness(i) = RelHostFitness(i)+0.5;
        ...
        %Mutation of winner given Mutation Bias
            for g = 1:NoGenes
                ...
                elseif HostWin == 0 && (rand < mutationRate) 
                    if ParPop(Win,g) ~= TargetString(g) && (rand < BPar)
                        ParPop(Win,g) = not(ParPop(Win,g)); 
                    elseif ParPop(Win,g) == TargetString(g) && (rand < (1-BPar))
                        ParPop(Win,g) = not(ParPop(Win,g));
                    end
                end
            end

            %Recalculate Host Pop's fitness, 
            [HostFit,HostFitness] = SimpleHostFitness(HostPop,NoHost,HostFitness,NoGenes,TargetString);

            %Recalculate par's fitness %Call on ParasiteFitness function
            [ParFit] = SimpleParFitness(ParPop,NoPar,NoGenes,TargetString); %Find simple parasite fitness 
            [ParasiteFit] = ParasiteFitness(NoPar,ParFit,lambda); 
        end

        for i = 1:NoPar
            for d = 1:TournSize
                TournComp(d) = floor(rand* NoHost + 1); %Select opponent indeces from parasite population
            end
            ...
            [HostFit,HostFitness] = SimpleHostFitness(HostPop,NoHost,HostFitness,NoGenes,TargetString);
            StoreHostFitnessSim(:,c) = HostFitness;

            %Recalculate par's fitness %Call on ParasiteFitness function
            [ParFit] = SimpleParFitness(ParPop,NoPar,NoGenes,TargetString); %Find simple parasite fitness by counting ones
            [ParasiteFit] = ParasiteFitness(NoPar,ParFit,lambda); 
            StoreParFitnessSim(:,c) = ParFit;
        ...
        bestHostFitness(c) = max(HostFit);
        bestParFitness(c) = max(ParasiteFit);
        
        relHostFit(c) = (sum(RelHostFitness))/NoHost;
        relParFit(c) = (sum(RelParFitness))/NoPar;
        
        StoreRelativeFitH(runs,c) = relHostFit(c);
        StoreRelativeFitP(runs,c) = relParFit(c);
...
% Set up figures
    figure(1)
    plot(bestHostFitness,'b*', 'MarkerSize', 2)
    hold on;
    plot(bestParFitness,'r.', 'MarkerSize', 2)
    title('Best Fitness')
...
function [AvgHostOnes] = AverageHostOnes(StoreTotalOnesHost,NoRuns) 
...
function [HostFit,HostFitness] = SimpleHostFitness(HostPop,NoHost,HostFitness,NoGenes,TargetString)
    for i = 1:NoHost
       HostFitness(i) = NoGenes - sum(abs(TargetString-HostPop(i,:)));
    end
...
function [ParFit] = SimpleParFitness(ParPop,NoPar,NoGenes,TargetString)
    for i = 1:NoPar
        ParFit(i) = NoGenes - sum(abs(TargetString-ParPop(i,:)));
    end 
end
